...
export function TransactionList({
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    ...
    return sortDirection === "asc" ? (
      <ChevronUp size={14} className="text-teal-600 dark:text-teal-400" />
    ) : (
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 dark:bg-gray-800/80 border-b border-gray-100 dark:border-gray-700">
            <tr>
              <th
                ...
                className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group select-none"
              >
              ...
              <th
                ...
                className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group select-none"
              >
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {dayGroups.map((group) => (
              <>
                {/* Date group header */}
                <tr key={`header-${group.dateKey}`} className="bg-gray-50/80 dark:bg-gray-700/40">
                      ...
                      <div className="flex items-center gap-4 text-xs font-medium">
                        ...
                        {onAdd && (
                          <button
                            ...
                            className="ml-1 p-1 text-gray-400 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-full transition-colors"
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {onEdit && (
                              <button
                                ...
                                className="text-gray-400 hover:text-teal-600 dark:hover:text-teal-400 transition-colors p-2 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-full"
                            ...
                            {onDelete && (
                              <button
                                ...
                                className="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full"
      ...
      <div className="block md:hidden">
        {dayGroups.map((group) => (
          <div key={`mobile-group-${group.dateKey}`}>
            {/* Mobile Date Header */}
            <div className="bg-gray-50/90 dark:bg-gray-700/60 px-4 py-3 border-b border-gray-100 dark:border-gray-700 sticky top-0 backdrop-blur-sm z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {group.transactions.map((transaction) => {
                ...
                return (
                  <div key={transaction.id} className="p-4 bg-white dark:bg-gray-800 active:bg-gray-50 dark:active:bg-gray-700">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            {transaction.bucketId && (() => {
                              const name = getBucketName(transaction.bucketId);
                              return (
                                <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium mt-1 ${name ? "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300" : "bg-gray-100 dark:bg-gray-700 text-gray-400 line-through"}`}>
                                  {name || "Deleted"}
                                </span>
                              );
                            })()}
                        ...
                        {(transaction.note || onEdit || onDelete) && (
                          <div className="flex justify-between items-end mt-2">
                            <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1 flex-1 mr-2">
                              {transaction.note}
                            </p>
                            ...
                            {(onEdit || onDelete) && (
                              <div className="flex gap-1 shrink-0">
                                {onEdit && (
                                  <button
                                    onClick={() => onEdit(transaction)}
                                    className="p-1.5 text-gray-400 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded"
                                  >
                                    <Pencil size={16} />
                                  </button>
                                )}
